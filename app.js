  ...
  return `
    <div class="page-stack">
      <section class="section-title">
        <div>
          <h1>每周 AI 新闻</h1>
          <p>每周汇总适合初中信息科技课堂导入的 AI 动态，保留来源链接、课堂关联点和主题标签。</p>
        </div>
      </section>
      ${
        news.length
      ...
      <section class="panel">
        ...
        <ul class="guide-list" style="padding: 0 26px 26px;">
          ...
          <li><span>2</span><div>写入 <code>data/ai-news.json</code>，每条包含标题、日期、来源、链接、摘要、课堂关联点和标签。</div></li>
...
function renderNewsCard(item) {
  return `
    <article class="news-card ${item.featured ? "featured" : ""}">
      <h3>${escapeHtml(item.title)}</h3>
      <p>${escapeHtml(item.summary)}</p>
      <div class="classroom-angle"><strong>课堂关联：</strong>${escapeHtml(item.classroomAngle || "待补充")}</div>
    ...
    <section class="resource-frame">
      <div class="empty-visual" aria-hidden="true">
        <div class="folder-stack">
          ...
          <div class="folder-row">筛选课堂关联</div>
